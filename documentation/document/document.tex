...
\paragraph*{Doe-het-zelf:} Welke weerstand $R$ moet de potentiometer hebben, wetend dat de ingangsspanning $V$ $200$ mV ( = $200 \cdot 10^{-3}$ V)is en we het vermogen $P$ willen beperken to 4 $\mu$W ( = $4 \cdot 10^{-6}$ W)? \textbf{Tip:} De formule voor elektrisch vermogen kan je vinden op pagina \pageref{eq:vermogen}.
\\.\dotfill \\.\dotfill \\.\dotfill \\.\dotfill \\
\begin{align}
    R = \ldots\ldots
\end{align}
...
In figuur \ref{fig:ges} vind je de audio-versterker. 
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.95\textwidth]{ges}
	\caption{Versterkerschakeling met de transistor.}
	\label{fig:ges}
\end{figure}

1) Vind de spanningsversterkingwet
2) Design de rest
