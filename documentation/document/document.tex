...
GSM, AUTOMATIC CARS, HEALTCARE, PS4, etc..
...
\begin{enumerate}
	\item de \textbf{spanning} (ook voltage of potentiaal genoemd) gemeten in volt [V], vaak aangeduid met de letter $V$ of $U$. Dit stelt het verschil in potentiële elektrische energie \textbf{tussen twee punten}.
...
\begin{itemize}
	\item twee weerstanden zijn \textbf{in serie}: ze hebben één gemeenschappelijke pin, dan is de vervangingsweerstand :
	\begin{align}
		R_{v} = R_1 +R_2.
	\end{align}
	\item twee weerstanden staan \textbf{parallel}: de twee weerstanden hebben beide pinnen gemeenschappelijk. Dan geldt voor de vervangingsweerstand 
	\begin{align}
...
Zoals al eerder vermeld, is voor de rest van het elektronisch netwerk een microfoon gelijk aan een spanningsbron. Stel dat we in serie met die spanningsbron twee weerstanden schakelt (figuur \ref{fig:volume}). We gaan aantonen de spanning $V_2$ een verkleinde versie is van het muzieksignaal $V_s$.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.4\textwidth]{weerstandsdeler}
	\caption{Volumeregeling: de weerstandsdeler}
	\label{fig:volume}
\end{figure}

\paragraph*{Doe-het-zelf:} probeer een formule te vinden voor $V_2$ waar alleen de weerstandwaarden $R_1$ en $R_2$ en de spanning $V_s$ in voorkomt. 
\textbf{Tips:} bereken eerst de stroom $I_2$ door gebruik te maken van de formule voor weerstanden in serie. Gebruik dan de wet van Ohm de spanning $V_2$ te berekenen.
